...
type Coupon struct {
	...
	Amount float64 `gorm:"column:amount;type:decimal(10,2);not null;default:0.00;comment:金额"`
	...
	MinPoint    float64 `gorm:"column:min_point;type:decimal(10,2);not null;default:0.00;comment:使用门槛；0表示无门槛"`
	...
	UseType  uint8 `gorm:"column:use_type;type:tinyint(4);unsigned;not null;default:0;comment:使用类型：0->全场通用；1->指定分类；2->指定商品"`
	// 公共字段
	BaseTime
}
