...
type ProductVertifyRecord struct {
	...
	VertifyMan string `gorm:"type:varchar(64);not null;default:'';comment:审核人"`
	Status     uint32 `gorm:"type:int(1);unsigned;not null;default:0;comment:审核后的状态：0->未通过；2->已通过"`
	Detail     string `gorm:"type:varchar(255);not null;default:'';comment:反馈详情"`
	// 公共字段
	BaseTime
}
