...
type OrderOperateHistoryRepo struct {
	*db.GenericDao[entity.OrderOperateHistory, uint64]
}
...
func NewOrderOperateHistoryRepo(conn *gorm.DB) *OrderOperateHistoryRepo {
	return &OrderOperateHistoryRepo{
		GenericDao: &db.GenericDao[entity.OrderOperateHistory, uint64]{
			DB: conn,
		},
	}
}
...
func (r OrderOperateHistoryRepo) GetByDBOption(ctx context.Context, pageNum uint32, pageSize uint32, opts ...db.DBOption) ([]*entity.OrderOperateHistory, uint32, error) {
	var (
